...
const leaderboard = [...staff].sort((a, b) => b.services - a.services).slice(0, 3);
        ...
        {leaderboard.map((s, i) => (
          <div key={s.id} className="bg-card rounded-xl border border-border p-5 shadow-card flex items-center gap-4">
            <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold ${
              i === 0 ? "bg-warning/10 text-warning" : i === 1 ? "bg-muted text-muted-foreground" : "bg-warning/5 text-warning/70"
            }`}>
